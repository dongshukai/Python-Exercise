        ...
        if row:
            ...
            for i in range(len(col_map_name)):
                ...
                if ccol in SELECT_FIELD_MAP and rrow in SELECT_FIELD_MAP[ccol]:
                    ...
                    if col_map_name[i] == 'meta_data_code':
                        meta_data_code = rrow
                    if col_map_name[i] == 'meta_data_name':
                        meta_data_name = rrow
                    row_val = rrow
                row_dict[col_map_name[i]] = row_val

            # 处理元数据的对应关系
            if meta_data_code in json_data['model_code_id_dict']['hr.payroll.meta.data']:
                row_dict['meta_data'] = json_data['model_code_id_dict']['hr.payroll.meta.data'][meta_data_code]
            elif meta_data_name in json_data['model_name_id_dict']['hr.payroll.meta.data']:
                row_dict['meta_data'] = json_data['model_name_id_dict']['hr.payroll.meta.data'][meta_data_name]
            else:
                # 说明不存在, 要新建
                json_data = add_xml_content_directir(json_data, 'hr.payroll.meta.data', {'name':meta_data_name, 'code':meta_data_code})
                row_dict['meta_data'] = json_data['model_code_id_dict']['hr.payroll.meta.data'][meta_data_code]
...
def write_file():
    ...
    if not file_name:
        file_name = 'Payroll-Payitem.xlsx'
    if not result_name:
        result_name = 'standard_group(sequence).xml'
    print "file name: ", file_name
    print "result name: ", result_name
    data = open_excel(file_name)
    if data:
        # TODO 这里的数据与make_xml_to_json的结果对应
        json_file_name = 'payroll_data.json'
        json_file = json.loads(open(json_file_name, 'r').read())
        excel_data = read_excel_data(data, json_file)
        if excel_data:
            # 写到XML文件中PG003
            payroll_payitem_data = ""
            excel_data = sorted(excel_data, key=lambda x: x['evaluation_sequence'])
            for i, t in enumerate(excel_data):
                # 文件中已经存在了sequence,这里不需要再赋值,但是由于Excel的float型数据,需要转换
                t['sequence'] = int(t['sequence'])
                t['evaluation_sequence'] = int(t['evaluation_sequence'])
                payroll_payitem_data += PAYITEM_DATA_CONTENT % (t)
            group_info = {
                'payroll_payitem_data': payroll_payitem_data,
                'group_name': '正常员工薪资组',
                'group_code': 'normal_pg',
                'is_standard_group': False,
            }
            xml_context = STANDARD_PAYROLL_STRUCTURE % (group_info)
            try:
